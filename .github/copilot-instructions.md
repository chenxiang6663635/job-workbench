...
共享仓库规则（诚实红线、三层分离、工程约定）的**唯一事实源是根目录 `AGENTS.md`**，请先阅读并遵守；本文只做摘要，与 `AGENTS.md` 冲突时以其为准。开发流程见 `docs/contributing.zh-CN.md`（完整中文细则，权威版，含隐私约定；英文伴读版 `docs/contributing.en-US.md`——必读节同步、治理节为摘要；短索引在根 `CONTRIBUTING.md`）。
