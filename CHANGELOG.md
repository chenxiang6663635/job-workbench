...
- **导出可以把材料投影成 Obsidian 笔记了（`jobws export --obsidian <目录> --notes`，2026-09-20）**：八张 CSV 之外，另把 `03_面试准备` / `04_知识库` / `00_事实库` 的 Markdown **原样**投影——保留目录层级、**全量读取不截断**（256 KB 截断只是只读端点的语义，带进导出会把几万字的速记截成半篇），frontmatter 记「来源目录 / 相对路径 / 标题」方便检索。**训练卡目录跳过**：那些卡导入后已在「题库/」下以 flashcard 形态出现，不重复投影。默认不带 `--notes` 时行为与之前完全一致（新增能力由新开关承载）。
