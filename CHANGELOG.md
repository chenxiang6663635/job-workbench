...
- **桌面端启动不再多开一个浏览器窗口**：打包版应用启动时，内置后端把自己当成「被用户双击运行」，顺手用系统浏览器又打开了一遍同一界面——桌面窗口旁边多冒一个浏览器。现在桌面壳会显式告知后端「由应用托管」，只有**独立运行后端**（web 形态）才自动开浏览器。源码开发模式不经过这条路径（走 `python -m uvicorn`），所以此前一直没被察觉；0.2.x 安装包带此问题。
