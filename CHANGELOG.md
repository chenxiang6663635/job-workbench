...
- **记录删除有了统一安全网（2026-09-21）**：邮件 / 面试 / 联系人 / 宣讲会 / Offer / 投递记录都能删了——界面点删除先看「将删的整行」差异表、确认后才落盘；落盘前整表快照写到**工作区之外**（删错可整份拷回）。**删投递不连带删关联记录**：邮件 / 面试等记录保留，只清「关联记录」外键（差异表逐条列明将解绑哪几条），变更时间线留一条「已删除」。邮件台账旧的「点两次直接删」已撤除——删除与全站其余写操作同走两段式。
