...
- **只读 MCP 服务（`jobws-mcp`）**：把工作台数据以 MCP 工具的形式暴露给支持 MCP 的宿主，三个**只读**工具——投递记录（`list_applications`）、岗位池（`list_jobs`）、看板摘要（`dashboard_summary`）。它是**独立的小包**：自带 Python 3.10+ 环境，用 `uvx` / `pipx` 拉起，不进桌面应用的依赖（主干后端仍是 Python 3.8 基线；两个依赖集互斥，这是刻意的隔离）。口径与命令行 / 网页端同源：漏斗、待办、逾期、静默提醒、健康度照搬看板，岗位评分照搬 `jd_score`，排序照搬追踪表——不另写一套判据。工作区只能落在「应用根」或「数据根」之内，**越界一律拒绝**（宿主可能由模型代传参数，少了这道检查等于给出任意目录的读取能力）。安装与宿主配置见 `mcp/README.md`。
