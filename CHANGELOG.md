...
- **界面双语（简体中文 / English）**：七个页面与桌面壳全部可切换，顶栏 `中文 / English` 切换、首次按系统语言、选择被记住（`<html lang>` 与窗口标题一并跟随）。数据层零变化——CSV / Markdown 里存的枚举取值（阶段、批次、结果等）仍保持中文，它们是与你历史数据和 CLI 共享的契约；界面文案由显示层映射成当前语言。关闭 #19。
