...
- **`web/start.ps1` 不再依赖「终端里激活了哪个 Python」**：它按 `-Py` → `JOBWS_PYTHON` → 仓库内 `.venv` → PATH 的顺序解析，并逐个验版本（≥3.9）与依赖（能 `import fastapi, uvicorn`），全不合格就给人话报错与三条修法。新增 `-CheckOnly` 只做预检。背景：终端自动激活的 conda 环境里的 `python` 可能是 3.8 或没装依赖的环境——那会让后端"起不来"或"起成半坏状态"，而原因看起来像功能坏了。
