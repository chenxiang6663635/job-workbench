...
- **无障碍：分段控件改用原生单选组（B15）**：岗位池的排序条与简历工坊的编辑模式原先借 `ui/tabs` 当"单选开关"用——Tabs 的触发器带 `aria-controls`，指向一个并不存在的内容面板（axe 的 `aria-valid-attr-value`，此前靠两条豁免压着）。换成新的 `ui/segmented`（原生 `input[type=radio]` + `role="radiogroup"`）后：**两条豁免清零**、四个页面 axe 的 serious/critical 归零；方向键切换与焦点环由浏览器与样式给出（本地实测：排序 dir→score、模式 标准→高级；焦点环可见）。**键盘走查**：七页各按 Tab 走一遍，14–18 个可聚焦元素，无键盘陷阱、焦点始终可见。
- **七个页面内容区同宽（设置页不再窄 45%）**：设置页此前自带 `max-w-2xl`——内容区 672px，其余六页 1232px，切 tab 时宽度整块跳变；现改为同宽并把四张卡排成两列（表单行长度仍在可读范围）。页面纵向间距也从两种（4 / 6）统一为一种。**新增机检**：UI 冒烟断言七个页面的内容区宽度**只允许一个取值**。
