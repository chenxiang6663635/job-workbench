...
- **打包桌面版的多工作区切换不再 404**：`?ws=` 相对名此前恒按**应用根**拼路径（注释写「两个根都允许」，但数据根分支实际不可达）——NSIS 安装版的工作区在系统用户目录，按安装目录拼只会指向空壳；现改为**数据根优先、应用根兜底**（与 `routers/workspace.py` 及 MCP 侧同口径）。开发 / 便携形态行为完全不变。
