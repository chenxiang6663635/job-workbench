...
> **例外（机器人依赖 PR，2026-09-13 定）**：`dependabot[bot]` 开的依赖升级 PR 以「**作者自审 + 四项 CI**」为门槛，**不要求第二轮独立审查**。
> - 理由：这类 PR 的 diff 是版本号与锁文件，第二轮评审的信息量极低；真正的验证者是 CI（构建、测试、UI 冒烟），而把「两轮」套在它上面只会催生走过场。
> - **边界（三条，缺一不可）**：① 只覆盖 **patch / minor** 升级；**major 升级不适用**（例：electron 33 → 44 是 11 个 major，必须走人工批次并做运行时/桌面冒烟——调研见 `docs/research/report_electron_33_to_44.md`）。② 仍须**改中文标题**（闸门照旧，机器人开不出合规标题）。③ 自审评论必须写明「改了什么 + 兼容性判断」，不能只写「CI 绿了」。
...
- 当前版本：**0.2.1**（tag 序列：`v0.1.0` 2026-09-08 → `v0.1.1` → `v0.2.0` → `v0.2.1`，实际值以 `web/electron/package.json` 为准）。后续按规则 bump 并打新 tag。
