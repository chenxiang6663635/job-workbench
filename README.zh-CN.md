...
界面**中英双语**：每一页都有中文与英文，顶栏的 `中文 / English` 可随时切换（首次按系统语言，选择会被记住）。下面截图是中文界面；英文界面是同样页面换了界面文字。仍然保留中文的只有三处：你自己录进工作区的内容、CSV / Markdown 里存的枚举取值（与 CLI 共享的数据契约）、CLI 自带帮助——都有意为之。
