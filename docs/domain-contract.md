...
内置插件不匹配你的专业（小众专业、交叉方向），或者你要按自己的真实经历校准
词典与锚点——不必照着本文档手写，让 AI 带你走：

1. 确保 AI 宿主已装 `jwb-domain-setup` 技能（随工作台分发；`jobws skills install` 一次装齐三类资产）；
2. 对 AI 说「帮我生成我专业的领域插件」，技能按**人确认制**流程走：采集你的真实经历 → 生成候选（全部标【待确认】）→ 词典逐层、锚点逐档由你确认 → 写盘前必过 `jobws lint domains`；
3. 产物只写你**工作区的 `config/`**——不碰本仓库、不进版本控制；想换回内置插件时，把 `config/` 里的四件套归档、重新 `jobws init --domain <id>` 即可。

**人确认制是「诚实红线」在插件侧的延伸**：AI 只产候选与初稿，词典词条必须对应
使用者真实做过的事（每个候选词附「能不能经得起两层追问」的自检问题），AI 不得
凭常识编词条、不得自行定层后直接写盘。

想把打磨好的插件**贡献回仓库**（成为所有人可用的内置插件）？回到第 1–4 节：
按目录结构与校验要求整理，提 PR 走贡献流程。
