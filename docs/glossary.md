...
| 术语 | 含义 | 出处 / 延伸 |
|---|---|---|
| **两段式（写入）** | 任何写入先出「预览 + 差异 + 令牌」，确认后再凭令牌落盘——全站唯一写通道 | contributing.zh-CN.md、`docs/four-ends.md` |
| **令牌** | 两段式第一步签发的一次性凭据：绑定工作区、带载荷指纹、10 分钟有效、用完即焚 | `mcp/README.md`、contributing.zh-CN.md |
...
| **shim（转发层）** | 旧 import 名的兼容文件：只做 `sys.modules` 转发 + 废弃告警，让旧路径平滑迁移 | contributing.zh-CN.md、CHANGELOG |
...
| **规模水位线** | 超规模文件登记的行数上限：只许变小；降到阈值以内时要求删条目（自洁） | `tools/size_allowlist.txt`、contributing.zh-CN.md |
| **自洁强制** | 水位降到阈值以内时，检查器报错要求删掉登记条目——防止清单腐化 | contributing.zh-CN.md「代码卫生」 |
| **单一发布节点** | 中间批次不 bump / 不 tag / 不 Release；全部批次做完后只发布一次 | contributing.zh-CN.md「版本号体系」 |
| **版本号** | 月粒度 CalVer `YY.MM.N`（如 `26.9.0`）：tag、CHANGELOG 段名、`package.json` 与界面「关于」显示同一个号；第三位 N = 当月第几发（hotfix 锁前两位只动 N） | contributing.zh-CN.md「版本号体系」 |
...
| **静默吞错** | `except: pass` / 空 `catch {}` 一类「出错无声」的写法——仓库禁用它，至少记日志 | contributing.zh-CN.md「代码卫生」 |
...
| **截断诚实** | 内容被截断时必须显式告知（如 256 KB 截断并提示），不许静默少给 | 使用手册、contributing.zh-CN.md |
| **四道门** | 新需求准入的四步评估流程 | contributing.zh-CN.md |
