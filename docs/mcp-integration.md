...
需要 Python **3.12+**（与领域包 `jobws-core` 同一条基线；工作台后端也是 3.12）。
...
本包原先的「sys.path 注入 + `JOBWS_REPO_ROOT`」硬闸已**整段删除**——装在哪都行。

两个附带说明：

- **安装顺序**：`jobws-core` 不在 PyPI（是本仓的包），要先装它再装 MCP：
...
- **两个操作仍需要仓库在侧**：`prep.toggle`（笔记勾选框写回）与 `init`（初始化工作区）
  ...
  `unknown_operation`（稳定错误码，不是崩溃）；其余十九个写操作（`track.*` /
  `talk.add` / `mail.add` / `interview.*` / `question.*`，以及七类删除与 `job.rename`）
  装包即用。
