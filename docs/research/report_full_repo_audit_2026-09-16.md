...
```
作键会与追踪表系统性失配。
（卡片里填的常是给人看的详细描述（如「奥克斯集团（空调事业部＝…）」），当
```

**核实**：`奥克斯` 在 `personal/05_投递追踪/tracker.csv`、`personal/01_岗位池/奥克斯_*/解析卡.md`（共 7 处）出现——是**用户真实投递过的公司**。而它同时出现在已推送到 GitHub 的 `jobs.py` 里。

**性质**：违反 `CONTRIBUTING.md:15`「禁止提交任何 `personal/` 内容：包括…真实公司名」。这是**唯一**一处此类泄漏；同文件/同仓库的 `TCL`（`url_infer.py:8`、`test_job_linking.py:136`）是刻意的虚构示例，`云帆智算`/`示例科技` 是通用 demo 数据，均合法。
