...
> **2026-09-16 注记**：本文是当时的实现记录，保留原貌。其后仓库经历多批演进——
> Python 基线已从 3.8 升到 3.12、依赖上限已放宽、界面双语化、主题与字体体系落地、
> 进展页扩展等；文中**依赖版本、Python 基线、页面 / 命令清单等章节不代表现状**，
> 现状以 `CONTRIBUTING.md`、`web/backend/requirements.txt` 与 `docs/usage-guide.md`
> 为准。架构层面的判断（同源数据、workspace 隔离、并发与安全立场）仍然成立。
