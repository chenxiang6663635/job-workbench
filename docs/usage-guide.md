...
- **Export as Obsidian notes** (CLI): `python tools/jobws.py export --obsidian <dir>` — one directory per CSV and **one Markdown note per row**, with all original columns in frontmatter (so Obsidian **Bases / Dataview** can filter and sort), question-bank notes carry spaced-repetition card syntax; output is timestamped, **never overwrites** a previous export, and **never touches the workspace** (a snapshot, not a sync)
