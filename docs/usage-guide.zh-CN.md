...
- **笔记**（只读）：`03_面试准备` 与 `04_知识库` 的 Markdown 直接读——左侧目录树（`README` 显示为「目录说明」、`_模板_`/`_示例_` 淡显、空文件与读不了的文件带标记、可按文件名过滤），右侧渲染成正文（标题 / 列表 / **勾选框** / 表格 / 代码块 / 引用），长文右侧有「本页大纲」，超 256 KB 截断并显式告知。**只读**——改内容用自己的编辑器，切回来 10 秒内自动刷新（并回到原文件）；勾选框目前只展示状态（打勾写回在后续批次）
