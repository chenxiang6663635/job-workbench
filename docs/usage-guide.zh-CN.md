...
拉取列表里的邮件有**三条去向**：点邮件正文 = 走「解析 → 建议 → 确认」更新投递状态；点 ✨ = 就地展开**解析建议**（时间 / 会议链接 / 建议阶段 / 对应岗位，每条都带原文出处与把握程度，逐条确认才写入——邀请类邮件**优先按日历附件解析**，比正文正则可靠）；点右侧的「＋」小图标 = **把这封邮件的元数据记入邮件台账**（主题 / 发件人 / 日期 / Message-ID）——之后到「进展 → 邮件」里补标签与关联，不用再手打一遍。

解析建议里，把握程度为「需核对」的取值（原文没写年份、相对日期如「明天」）必须勾选「我已核对取值」才允许写入；会议链接卡片可以直接复制或打开，写进台账后也能在「进展 → 邮件」里再次打开或复制。解析建议只在**拉取流程**里出现——手工粘贴的正文没有日历附件，仍走「粘贴邮件」的原有解析对话框。配置了 BYOK Provider（设置 → Provider）之后，底部还会出现**可选 AI 增强**：填模型名后手动跑一次，同一条建议链路、一律标「AI」且同样需要你核对——**AI 只产建议，绝不直接写入**。
