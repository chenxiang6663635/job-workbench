...
解析建议里，把握程度为「需核对」的取值（原文没写年份、相对日期如「明天」）必须勾选「我已核对取值」才允许写入；会议链接卡片可以直接复制或打开。配置了 BYOK Provider（设置 → Provider）之后，底部还会出现**可选 AI 增强**：填模型名后手动跑一次，同一条建议链路、一律标「AI」且同样需要你核对——**AI 只产建议，绝不直接写入**。
