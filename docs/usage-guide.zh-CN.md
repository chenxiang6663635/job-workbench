...
3 场宣讲会 / 6 道题 / 6 封邮件 / 7 张解析卡 / 1 份简历），公司与人名全是假的，可以直接上手体验或截图；
...
**到点提醒的完整口径**（桌面端）：设置页可调「提前几天提醒」（3 / 5 / 7 天，默认 3）。满足任一条就发一条系统通知：**已过期**（含「截止日期已过仍待投」与「下次动作日期已过」两类，按过期天数排在前）、**提前 N 天内的待办**（今天到期的与还剩几天的在正文里分开写）、**近 7 天宣讲会**。去重是**按事项、每天一次**——同一条今天报过就不再报，但**今天新出现的**照样会报（旧口径"一天只报一次"会把新出现的截止项漏掉）。**点通知会跳到追踪表并自动展开那一条**，不用自己找。**应用内容区顶部还有一条常驻提醒条**（读同一个端点、受同一个开关约束）：打开应用就能看到「N 条已过期 / 近 N 天 N 条待办 / N 场宣讲会」，点一下跳到对应页面；没有到点事项时它不出现、也不占位。**关掉开关，通知与提醒条一起停**；关窗即停，没有后台服务、不建托盘。
