...
正文里「请在 X 前完成在线测评 / 笔试」这类句子会被识别为**截止事项**（第五类候选）：卡片上带任务名（如"完成在线测评"）与日期，确认后**同时写入**投递记录的「下次动作日期」与「下次动作」。这两个字段正是**到点提醒**（看板「近七天待办」与系统通知）读的，所以倒计时会自动出现，不需要额外设置；相对日期支持「今天 / 明天 / 后天 / 大后天 / 本周X / 下周X」（后两者按周一为起点），算出的日期若已过去，那就是"已过期"信号，按「需核对」提示你确认。
