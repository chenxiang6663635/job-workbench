...
- **训练**（题库的「练」入口）：抽题 → **盲答**（答案默认折叠）→ 展开对答案 → 自评三态 / 标错题 → 下一题。三种模式：重练队列（错题 ∪ 今日待复习）、只错题、随机；可按领域 / 科目 / 状态 / 关键词收窄。写操作零新增（自评改「状态」、标错题改「标签」，都先看差异再确认）；命令行同效：`jobws bank drill --mode due|wrong|random --n 5`
