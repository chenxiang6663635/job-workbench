...
拉取列表里的邮件有**三条去向**：点邮件正文 = 走「解析 → 建议 → 确认」更新投递状态；点 ✨ = 就地展开**解析建议**（时间 / 截止事项 / 会议链接 / 建议阶段 / 对应岗位，每条都带原文出处与把握程度，逐条确认才写入——邀请类邮件**优先按日历附件解析**，比正文正则可靠）；点右侧的「＋」小图标 = **把这封邮件的元数据记入邮件台账**（主题 / 发件人 / 日期 / Message-ID）——之后到「进展 → 邮件」里补标签与关联，不用再手打一遍。
