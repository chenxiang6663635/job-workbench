# -*- coding: utf-8 -*-
"""jobws_core.tracker 包：求职工作台的投递追踪领域层（7 张表）。
...
**本包与旧路径的关系**（2026-09-19 批 6 第二批）：领域层从仓库的 `tools/tracker/`
搬进本包；本仓的 CLI 子模块 `_cli*.py` 按用户拍板**留仓**，由
`tools/tracker/__init__.py` 那个 shim 挂回 `tracker.*` 命名空间——
所以下面的 `_HOME` 只列领域模块，`__all__` 里仍留着 CLI 侧的历史名字
