...
_LOCK_KINDS = {
    ...
    # 写语义，跨目录互斥的代价可忽略。锁文件 `.` 开头，列表遍历与指纹天然排除。
    "prep": ("03_面试准备", ".prep.lock"),
}

# 默认纳入指纹的业务目录（读不到就跳过，不报错——空工作区也要有稳定指纹）
# 04_知识库 于 2026-09-18 补入：笔记页（只读浏览）依赖指纹实现"外部编辑后刷新"，
# 不补的话在 Obsidian 改完速查卡切回来还是旧内容；00_事实库 同日随素材库去债
# 批补入（素材库同为只读浏览：外部生成/更新的事实卡，切回来也要能看到）。
DEFAULT_TRACKED_DIRS = ("00_事实库", "05_投递追踪", "01_岗位池", "02_简历工坊",
                        "03_面试准备", "04_知识库")
