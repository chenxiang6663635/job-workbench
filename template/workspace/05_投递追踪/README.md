...
| 6 | 来源 | 应届生求职网 / 牛客 / 企业校招官网 / 学校就业网 / 内推 / 宣讲会 / 招聘会 / 其他 | `jobws track check` 会校验 |
| 7 | 链接 | URL | 岗位页原始链接（`add --link` / `update --link`），可空 |
| 8 | 截止日期 | YYYY-MM-DD | 空表示未知 |
| 9 | 投递日期 | YYYY-MM-DD | 未投则空 |
| 10 | 当前阶段 | 见下 | |
| 11 | 状态原因 | 文本 | 进入终态时必填（复盘归因用） |
| 12 | 下次动作 | 文本 | |
| 13 | 下次动作日期 | YYYY-MM-DD | 驱动「近 7 天待办」 |
| 14 | 简历版本 | 与 `resume_<版本>.html` 对应 | |
| 15 | 评分 | 0–100 | 来自 JD 解析 |
| 16 | 归档目录 | 相对路径 | |
| 17 | 备注 | 文本 | |
...
`update` 只能改阶段、状态原因、下次动作、日期、备注、评分、链接。**公司与岗位不可改**——需要改则新建一条，把旧的标为已放弃。
