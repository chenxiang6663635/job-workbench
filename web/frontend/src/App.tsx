...
export default function App() {
  ...
  // 首启序列（列工作区 → 激活 → 探活）与「重试」入口都在 hook 里（UX-1）
  const { online, workspaces, workspaceReady, currentWs, boot, switchWorkspace } =
    useBackendBoot();
      ...
      <main className="relative mx-auto max-w-7xl px-6 pb-16 pt-24">
        {online === false ? (
          <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-6">
            ...
            {/* UX-1：后端起好了却停在离线屏时，重试即可，不用整页刷新 */}
            <Button size="sm" variant="outline" className="mt-4" onClick={boot}>
